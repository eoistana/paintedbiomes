...
public class ChunkGeneratorPaintedBiomes extends ChunkGeneratorOverworld {
    ...
    public void setBlocksInChunkWithPaintedHeight(int x, int z, ChunkPrimer primer)
    {
        ...
        for (int gx = 0; gx < 16; ++gx)
        {
            ...
            {
                int blockX = gx + chunkX;
                int blockZ = gz + chunkZ;
                int alpha = this.featureHandler.getAlphaAt(blockX, blockZ);
                if(alpha<255)
                {
                    if(oldPrimer == null)
                    {
                        oldPrimer = new ChunkPrimer();
                        setBlocksInChunk(x, z, oldPrimer, this.biomesForGeneration);
                        Biome[] biomes = this.world.getBiomeProvider().getBiomes(null, x * 16, z * 16, 16, 16);
                        replaceBiomeBlocks(x, z, oldPrimer, biomes);
                    }
                }
                int h = this.featureHandler.getHeightAt(blockX, blockZ, this.blockHeights[gz + gx*16]);
    ...
    /** 
     * Tweaked old chunk generation
     */
    public void setBlocksInChunk(int x, int z, ChunkPrimer primer, Biome[] biomesForGeneration)
    {
        this.generateHeightmap(x * 4, 0, z * 4, biomesForGeneration);
    ...
    private static float[] biomeWeights = initBiomeWeights();
    
    private static float[] initBiomeWeights()
    {
    ...
    private void initOldGenerationVariables(World worldIn)
    {
        ...
        this.blockHeights = new int[256];
    }
    
    private void generateHeightmap(int x, int y, int z, Biome[] biomesForGeneration)
    {
        double[] depthRegion = this.depthNoise.generateNoiseOctaves(null, x, z, 5, 5, (double)this.settings.depthNoiseScaleX, (double)this.settings.depthNoiseScaleZ, (double)this.settings.depthNoiseScaleExponent);
        float f = this.settings.coordinateScale;
        float f1 = this.settings.heightScale;
        double[] mainNoiseRegion = this.mainPerlinNoise.generateNoiseOctaves(null, x, y, z, 5, 33, 5, (double)(f / this.settings.mainNoiseScaleX), (double)(f1 / this.settings.mainNoiseScaleY), (double)(f / this.settings.mainNoiseScaleZ));
        double[] minLimitRegion = this.minLimitPerlinNoise.generateNoiseOctaves(null, x, y, z, 5, 33, 5, (double)f, (double)f1, (double)f);
        double[] maxLimitRegion = this.maxLimitPerlinNoise.generateNoiseOctaves(null, x, y, z, 5, 33, 5, (double)f, (double)f1, (double)f);
        int i = 0;
        int j = 0;
        ...
        for (int k = 0; k < 5; ++k)
        {
            for (int l = 0; l < 5; ++l)
            {
                ...
                for (int l1 = 0; l1 < 33; ++l1)
                {
                    ...
                    double d2 = minLimitRegion[i] / (double)this.settings.lowerLimitScale;
                    double d3 = maxLimitRegion[i] / (double)this.settings.upperLimitScale;
    ...
    /**
     * End tweaked old chunk generation
     */
    ...
    public void populate(int x, int z)
    {
        ...
        boolean paintedStructureFlag = false;
        ChunkPos chunkpos = new ChunkPos(x, z);

        net.minecraftforge.event.ForgeEventFactory.onChunkPopulate(true, this, this.world, this.rand, x, z, villageFlag);

        if(this.featureHandler.isChunkInsideBounds(x, z))
        {
            //PaintedBiomes painted structures::
            paintedStructureFlag = generatePaintedStructures(x, z);
        }
        ...
        if (this.mapFeaturesEnabled)
        {
            ...
            if (this.settings.useVillages)
            {
                villageFlag = this.villageGenerator.generateStructure(this.world, this.rand, chunkpos);
            }
        ...
        boolean flag = villageFlag || paintedStructureFlag;
    ...
    private boolean generatePaintedStructures(int x, int z)
    {
        boolean flag = false;
